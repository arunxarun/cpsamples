...
public class GroupViewsByUser extends Configured implements Tool {
	...
	enum GroupByUserCounters {
		...
		REDUCE_CONTEXT_WRITE_IO_EXCEPTION, 
		REDUCE_CONTEXT_WRITE_INTERRUPTED_EXCEPTION, 
		BAD_REDUCE_INPUT_FORMAT
	}
	/**
	 * the key the user, the value is the URI
	 *
	 */
	public static class GroupViewsByUserMapper extends Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(final LongWritable key, final Text value,  final Context context) {
			...
			try {
				...
				context.getCounter(GroupByUserCounters.MAP_CONTEXT_WRITE_IO_EXCEPTION).increment(1);
				e.printStackTrace();
			} catch (InterruptedException e) {
				context.getCounter(GroupByUserCounters.MAP_CONTEXT_WRITE_INTERRUPTED_EXCEPTION).increment(1);
				e.printStackTrace();
			}
	 ...
	 *
	 */
	public static class GroupViewsByUserReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> input, Context context) {
			...
			LOGGER.debug("reduce: key = "+key.toString()+", value = "+viewBuffer);
			...
			try {
				...
				LOGGER.error(e);
				context.getCounter(GroupByUserCounters.REDUCE_CONTEXT_WRITE_IO_EXCEPTION).increment(1);
			} catch (InterruptedException e) {
				LOGGER.error(e);
				context.getCounter(GroupByUserCounters.REDUCE_CONTEXT_WRITE_INTERRUPTED_EXCEPTION).increment(1);
	...
	public int run(String[] args) throws Exception {
		...
		try {
			...
			job.setMapOutputValueClass(Text.class);  // this one matches the exected map output!
			...
			LOGGER.error("Overall Job did not complete succesfully" + className, e);
			throw e;
		}
		return result;
